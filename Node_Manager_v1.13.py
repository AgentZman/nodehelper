...
#create empty file so gui not blocked with exception.
file_path = "alert.txt"
...
def find_files(filename, root_dir): 
    ...
    matching_files.sort(key=lambda f: int(''.join(filter(str.isdigit,f))))            
    return matching_files
# for reference - sorted(matching_files, key=lambda i: int(os.path.splitext(os.path.basename(i))[0]))
...
def manage_nodes():
    ...
    #stop alert loop but keep status loop etc.
    def monitor():
        ...
        while True:
            ...
            for i in range(len(status_num)):
                status_num[i].invoke()
                if status_num[i]["bg"]=="chartreuse2" and int_vars[i].get()==0:
                    data += f"{node_label[i]['text']}= ON  (alert enabled)"+"\n"
                elif status_num[i]["bg"]=="chartreuse2" and int_vars[i].get()==1:
                    data += f"{node_label[i]['text']}= ON  (alert disabled)"+"\n"
                elif status_num[i]["bg"]=="firebrick2" and int_vars[i].get()==0:
                    data += f"{node_label[i]['text']}= OFF (alert enabled)"+"\n"
                    alert_stop=True
                elif status_num[i]["bg"]=="firebrick2" and int_vars[i].get()==1:
                    data += f"{node_label[i]['text']}= OFF (alert disabled)"+"\n"
            
            if alert_stop == True:
                alert_count+=1
                #print("count ", alert_count)
                if alert_count==1:
                    off_message(data)

            if alert_stop >17280: #every 24hr reset
                alert_stop=0                    
            time.sleep(20)       
    ...
    #alert.txt for ntfy subscription info    
    ...
    def save_checkbox_state(var):
        nonlocal alert_count
        nonlocal alert_stop
        with open("checkbox_state.txt", "w") as f:
            for i in range(len(int_vars)):
                f.write(str(int_vars[i].get())+"\n")
            alert_count=0
            alert_stop=0

    def load_checkbox_state():#needs work to read new lines. 
        try:
            with open("checkbox_state.txt", "r") as f:
                for i, line in enumerate(f):
                    int_vars[i].set(line.strip())
                    #print(line.strip())
        except FileNotFoundError:
            pass  # File doesn't exist, so do nothing
    ...
    for i, file_path in enumerate(found_files):
        ...
        checkbox[i] = tk.Checkbutton(window, text=f"Disable Alert", activebackground="goldenrod", bg=freedbg, variable=var, command=partial(save_checkbox_state, var))
        checkbox[i].place(y=node_y+3, x=node_x+473)
        #print(status_list)
        node_y=node_y+35
...
7-Set permissions(freedomcoind, freedomcoin-cli)=0o755\n\
